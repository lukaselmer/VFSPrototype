...
\begin{enumerate}
  ...
  \item Single inderection: The next "block reference" references to one indirect node, which reference to (block\_size / block\_reference\_size) oher file content blocks each.
  \item Double indirection: The next "block reference" references to one inderect node, which reference to $block\_size / block\_reference\_size$ indirect nodes each, which refernce to $block\_size / block\_reference\_size$ other file contents each.
  \item Triple indirection: The next "block reference" references to one inderect node, which reference to $block\_size / block\_reference\_size$ indirect nodes each, which reference to $block\_size / block\_reference\_size$ indirect nodes each, which refernce to $block\_size / block\_reference\_size$ other file contents each.
\end{enumerate}
\end{enumerate}
...
Iff the file type is a directory, than the Index Node contains the references to the objects (files or directory). If the amount of directories is smaller or equal to (block\_size - 256 bytes) / block\_reference\_size, 
...
As mentioned above already, the disk can grow or shrink by changing the amount of blocks.
...
Shrinking on the other hand is more difficult, because the last block could be occupied, even if the disk is not full. Therefore the last block(s) has to be moved to a free block, and the reference to this block has to be adjusted. This is very complex to implement efficient and therefore not implemented in this project (yet).\\
